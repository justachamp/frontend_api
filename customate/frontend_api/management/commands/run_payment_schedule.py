...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Submitting regular payment: user_id=%s, payment_account_id=%s, currency=%s, "
                          "payment_amount=%s, additional_information=%s, payee_id=%s, funding_source_id=%s" % (
                              str(s.user_id), str(s.user.account.payment_account_id), str(s.currency.value), int(s.payment_amount),
